...
top_bound, left_bound, right_bound, lower_bound = 720, 0, 1280, 0
args = ap.parse_args()
...
# Kelly: this determines "what range of colors seen in the diff image can be used to
#        make the thresh image
#
#
...
def returnLargeContour(frame,totalVideoPixels):
	...
	for z in contours:
		try:
			...
			if area > (totalVideoPixels*0.00025) and area < (totalVideoPixels*0.0021701):#and aspect_ratio <= 3.5 and aspect_ratio >= 0.3:
...
def find_tank_bounds(image,name_of_trial):
	...
	mask = cv2.inRange(hsv,np.array([0,0,100]),np.array([225,88,255]))
...
while(cap.isOpened()):
	...
	difference = cv2.absdiff(hsv, hsv_initial)
	...
	masked = cv2.resize(masked,(0,0),fx=0.35,fy=0.35)
	cv2.imshow('thresh',masked)
	difference = cv2.resize(difference,(0,0),fx=0.35,fy=0.35)
	cv2.imshow('diff',difference)
	# cv2.imwrite("diff_image.jpg", difference)
